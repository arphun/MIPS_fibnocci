...
# Output is $s3 (in integer)
	move $a0, $s1
	jal hw3 # $v0 is the output of T(n)
	move $s3, $v0
	j result

#STEP4: implement recursive function to solve the equation 
# $s1 :  integer n     $s2 :  integer c
#if n <=1 
#   return c
#else
#	return 2*hw3(n/2)+ C*n 
hw3:
	bge $a0, 2, recurse
	move $v0, $s2
	jr $ra
...
recurse:
	addi $sp, $sp, -8
	sw $ra, 0($sp)
	sw $a0, 4($sp) # a0 means n 

	addi $t0, $zero, 2 
	div $a0, $t0
	mflo $a0
	jal hw3  #now v0 is T(n/2)
	mult $v0, $t0 
	mflo $v0	# v0 = 2*T(n/2)
	lw $a0, 4($sp)
	mult $s2, $a0
	mflo $v1	# v1 = c*n
	add $v0, $v0, $v1 # v0 = 2*T(n/2)+c*n
	lw $ra, 0($sp)
	addi $sp, $sp, 8
	jr $ra
